...
**Load this when:** Running weekly hiring pipeline audits (automated Mon 10:30am + Fri 3pm)  
**Token cost:** ~300 tokens to load + ~800 per report to generate  
**Prerequisites:** Access to Markaz DB, Gmail (calendar, emails), all open positions tracked  
**When to load more:** Need to query pipeline? Load [database-queries.md](database-queries.md) | Need to schedule automation? Check CronCreate docs
...
**Similar skill:** [hiring_decision_brief.md](hiring_decision_brief.md) — also checks Markaz + Gmail + Calendar, but for a single position in depth; this skill is shallower but continuous across all positions.
