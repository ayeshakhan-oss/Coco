"""
SMG Case Study Evaluation Report (Job 42) — SINGLE PDF — PILOT to Ayesha.

Ayesha 2026-08-18: "can you send in one pdf report" — the two-part email split existed
only because Gmail clips bodies over ~100KB. A PDF removes that constraint entirely and is
forwardable to the hiring manager as one artefact.

CONTENT IS THE SAME EVALUATION as send_smg_case_study_evaluation_v2_pilot.py, imported
from it so there is ONE source of truth for the candidate data. Nothing is re-scored here.

Formatting per memory/feedback_pdf_formatting.md: ReportLab, TA_JUSTIFY on all body text.
Georgia registered from C:\\Windows\\Fonts so that >=, x, ->, em dashes render properly
(Helvetica base-14 cannot).
...
SUBJECT = "[PILOT - ] SMG Case Study - Full Evaluation Report (single PDF) | Job 42"
OUT_PDF = os.path.join(ROOT, "output", "reports",
                       "SMG_Case_Study_Evaluation_Report_2026-08-18.pdf")

# ---- single source of truth: import the evaluated content ----
_spec = importlib.util.spec_from_file_location(
    "_v2", os.path.join(os.path.dirname(__file__), "send_smg_case_study_evaluation_v2_pilot.py"))
...
def stat_boxes():
    data = [("6", "Shortlisted", NAVY), ("16", "Documents read", MIDBLUE),
            ("595k", "Characters", GREEN), ("1", "Not assessable", RED)]
    cells = []
    for n, l, c in data:
...
def header_footer(canvas, doc):
    ...
    canvas.drawString(21 * mm, h - 17.5 * mm, "SMG Case Study \u2014 Full Evaluation Report")
    canvas.setFillColor(colors.HexColor("#c3d0e6"))
    canvas.setFont("Georgia", 8)
    canvas.drawString(21 * mm, h - 22.5 * mm,
                      "Job 42  \u00b7  Senior Manager Growth  \u00b7  18 August 2026")
...
def build_pdf(path):
    ...
    doc = BaseDocTemplate(path, pagesize=A4,
                          ...
                          title="SMG Case Study - Full Evaluation Report (Job 42)",
                          author="Taleemabad Talent Acquisition")
    frame = Frame(doc.leftMargin, doc.bottomMargin, doc.width, doc.height, id="f")
    doc.addPageTemplates([PageTemplate(id="all", frames=[frame], onPage=header_footer)])

    intro = (
        "This is the detailed version, written so that reading it replaces reading the "
        "submissions. Every document was re-sourced and read end to end for this report "
        "\u2014 16 files, 595,215 characters, every narrative page and every spreadsheet tab. "
        "Each candidate is covered assignment by assignment, with what they found, the numbers "
        "they used, the experiments they designed, how they would measure the loop, their "
        "stalled-deal probability, and what is thin. <b>Scores are unchanged from the 17 August "
        "report</b> \u2014 this is a deeper write-up of the same evaluation, not a re-score.")
    ...
    story = [stat_boxes(), Spacer(1, 10), Paragraph(intro, BODY), Spacer(1, 12),
             ...
             Paragraph('<font size="7.6" color="#6b7a90">Dimensions scored 1\u20135. Weights: '
                       ...
                       '15% \u00b7 Discipline 10% \u00b7 Signal 10%. All six scored 70+ and all '
                       'six were invited to debrief on 18 August.</font>', TCELL),
...
def main():
    ...
    print("  structural checks passed: all 6 candidates, probes and method section present")
    print("  NOTE: structural != visual. Ayesha must eyeball the PDF (CLAUDE.md Rule 14).")

    body = (
        "Hi Ayesha,\n\n"
        "The full SMG case study evaluation, as one PDF - all six shortlisted candidates, "
        "covered assignment by assignment.\n\n"
        f"{len(d)} pages. Every submission was re-sourced and read end to end for this: "
        "16 files, 595,215 characters, every page and every spreadsheet tab. Scores are "
        "unchanged from the 17 August report - this is a deeper write-up, not a re-score.\n\n"
        "Two things I could not assess and have stated in the report: Shahmir's reflection is "
        "the .m4a voice note, so one of his six dimensions carries no evidence and his 98 stays "
        "provisional; and Arooj's 60-day timeline is an image, so it was not read.\n\n"
        "Please eyeball the layout before this goes to the hiring manager - I have no PDF "
        "viewer here, so I can only verify the contents structurally, not how the pages look.\n\n"
        "Coco")
    ...
    safe_sendmail(s, SENDER, RECIPIENTS, msg.as_string(),
                  context="smg_case_study_evaluation_single_pdf")
    s.quit()
