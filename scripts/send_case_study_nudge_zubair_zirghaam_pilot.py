#!/usr/bin/env python3
"""
Case Study Reminder — Zubair Hussain (GM-Karachi, app 3792) + Zirghaam Ahmad (GM-Karachi,
app 3830). Ayesha's brief 2026-08-17: send reminders to both.

VERIFIED (IMAP, Ayesha's mailbox):
  Zubair   - GM case study sent Fri 2026-08-07 21:59 PKT. No submission. Standard nudge.
  Zirghaam - sent Fri 2026-08-07 22:13 PKT BUT the email was labelled "Senior Manager
             Growth" and carried the SMG Execution Sprint doc (1Ygsscl6...). He is a
             GM-Karachi candidate with no SMG application -> WRONG BRIEF.
             He REPLIED Sat 2026-08-08: away with family in the Northern Areas, patchy
             internet, asked to submit after 12 Aug, offered to work sooner if needed.
             Nobody answered him. Ayesha 2026-08-17: send the standard nudge to both
             anyway - her call, flagged and noted.

Design/wording: locked interview-invite template FINAL_2026_05_13, no CTA button.
...
# Ayesha 2026-08-17: nudge BOTH. PILOT first, live only on her per-candidate approval.

CANDIDATES = [
    {"first": "Zubair", "full": "Zubair Hussain", "email": "zubair.hafiz8@gmail.com",
     "position": "Growth Manager - Karachi", "sent_day": "on Friday, 7 August"},
    {"first": "Zirghaam", "full": "Zirghaam Ahmad", "email": "zirghaam.ahmad96@gmail.com",
     "position": "Growth Manager - Karachi", "sent_day": "on Friday, 7 August"},
]
                                ...
                                <tr>
                                    <td style="padding:44px 64px 52px 64px; text-align:left;">
                                        ...
                                        <p>
                                            ...
                                            {sent_day}. We just wanted to check in, as we haven't received your submission yet.
                                        </p>
                                        <p>
...
def build_message(candidate, pilot):
    subject = f"Checking In: Your Case Study for {candidate['position']} - {candidate['full']}"
    if pilot:
        subject = f"[PILOT - ] {subject}"
    html = (HTML_TEMPLATE
            .replace("{first_name}", candidate["first"])
            .replace("{position}", candidate["position"])
            .replace("{sent_day}", candidate["sent_day"]))
...
def main():
    ...
    for c in CANDIDATES:
        ...
        safe_sendmail(server, SENDER, recipients, msg.as_string(),
                      context=f"{mode} case study nudge -> {c['full']} ({c['position']})")
