"""
Create SEPARATE Google Sheet using Drive API (bypasses scope limitations)
Then populate it using Sheets API
"""
...
print("\n" + "="*80)
print("Creating SEPARATE Google Sheet using Drive API")
print("="*80)

# Step 1: Create file via Drive API
print("\n[STEP 1] Creating new Google Sheets file via Drive API...")

file_metadata = {
    'name': 'Senior Manager Growth - Talent Slate (2026-06-03)',
...
try:
    ...
    print(f"[ERROR] Failed to create via Drive API: {e}")
    exit(1)

# Step 2: Add candidate data
print("\n[STEP 2] Populating candidate data...")
...
print("\n" + "="*80)
print("[SUCCESS] SEPARATE SHEET CREATED VIA DRIVE API")
print("="*80)
...
try:
    ...
    for i, row in enumerate(roles_rows):
        if row and len(row) > 0 and 'Senior Manager Growth' in row[0]:
            ...
            print(f"[FOUND] SMG at row {i+1}, updating...")
            ...
            updated = True
            print(f"[OK] Master roles sheet updated")
            break

    if not updated:
        print("[INFO] Adding new SMG entry...")
        new_row = [
    ...
    print(f"[WARNING] Could not update master: {e}")
    print(f"Manual link: {sheet_url}")
