#!/usr/bin/env python3
"""
CPD Coach Values Interview Invites - LIVE SEND
Send to candidates with CC to Ayesha + Asma
"""
...
CC_ADDRESSES = ["ayesha.khan@taleemabad.com", "asma.zaheer@niete.edu.pk"]

# Pre-approve all candidate addresses
...
try:
    ...
    for candidate in CANDIDATES:
        ...
        subject = "Values Interview Invite: " + candidate['name'] + " – " + POSITION
        msg['Subject'] = subject
        msg['From'] = SENDER
        ...
        recipients = [candidate['email']] + CC_ADDRESSES
        ...
        safe_sendmail(
            ...
            recipients,
            msg.as_string(),
            context="cpd_coach_values_interview_live_" + candidate['name'].lower().replace(' ', '_')
        )

        print(f"[SENT] {candidate['name']} ({candidate['email']})")
        print(f"       CC: {', '.join(CC_ADDRESSES)}")

    server.quit()
    print(f"\n[COMPLETE] All 3 invites sent LIVE")
    print(f"Position: CPD Coach")
    print(f"Sent to: Yumna Sohail, Namal Javaid, Rosheen Naeem")
