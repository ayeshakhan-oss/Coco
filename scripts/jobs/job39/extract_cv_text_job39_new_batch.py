"""
Extract CV text for Job 39 (Growth Manager - Lahore) new-status applicants.
Batch of 2026-08-12 screening. Saves text to output/cv_texts_job39_new_batch/.
...
OUTPUT_DIR = r"c:\Agent Coco\output\cv_texts_job39_new_batch"
...
def main():
    apps = q("""
        ...
        WHERE a.job_id = 39 AND a.status = 'new'
        ORDER BY a.id
    """)

    os.makedirs(OUTPUT_DIR, exist_ok=True)
    summary = []
    print(f"\n{'='*60}\nJob 39 GM-Lahore — new batch CV extraction ({len(apps)} apps)\n{'='*60}\n")
