...
Case Study Debrief Invitation — Rosheen Naeem & Mehwish
TO = candidate email | CC = hiring@taleemabad.com, ayesha.khan@taleemabad.com, jawwad.ali@taleemabad.com
...
CANDIDATES = [
    {"name": "Rosheen", "email": "rosheen.naeem@gmail.com"},
...
def send():
    ...
    for candidate in CANDIDATES:
        html = build_html(candidate["name"])
        ...
        msg["CC"]      = "hiring@taleemabad.com, ayesha.khan@taleemabad.com, jawwad.ali@taleemabad.com"
        msg["Subject"] = SUBJECT

        recipients = [candidate["email"], "hiring@taleemabad.com", "ayesha.khan@taleemabad.com", "jawwad.ali@taleemabad.com"]

        with smtplib.SMTP("smtp.gmail.com", 587) as server:
            server.starttls()
            server.login(SENDER, PASSWORD)
            safe_sendmail(server, SENDER, recipients, msg.as_string(),
                          context=f'job36_debrief_{candidate["name"].lower()}')

        print(f"Debrief invite sent to {candidate['name']} ({candidate['email']})")
