"""
Resubmit Laiba Ahmad scorecard with all metadata fields (matching Meer's process)
...
def main():
    ...
    print("[STEP 1] Clearing current scorecard for Laiba Ahmad (app 1389)...")
    cur.execute("""
        UPDATE applications
    ...
    print("[STEP 2] Resubmitting full scorecard with all metadata...")
    cur.execute("""
        UPDATE applications
        ...
        WHERE id = %s
    """, (
        json.dumps(SCORECARD_LAIBA_AHMAD),
        "pass",
        interview_date,
        "Ayesha Khan",
        "Right Seat",
    ...
    print("[OK] Laiba Ahmad scorecard resubmitted with all metadata fields")

    conn.commit()
    cur.close()
    conn.close()
    print("[OK] Complete: Laiba Ahmad scorecard resubmitted to Markaz.")
