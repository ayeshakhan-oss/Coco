"""
Fix: Arsalan's scorecard was incorrectly written to app 1851 (Shahzad Saleem Abbasi).
This script:
1. Clears Shahzad's record (app 1851) back to shortlisted, no scorecard
2. Writes Arsalan's scorecard to his correct app_id (1357)
"""
...
ARSALAN_SCORECARD = {
    ...
    "finalComments": (
        ...
        "Note: Values 1-2 evaluated with leniency due to audio conditions (background noise, connectivity "
        "issues, location change mid-call). Values 3-6 evaluated strictly on transcript content."
    ),
...
def main():
    ...
    # Step 1: Clear Shahzad's contaminated record
    print(f"Clearing Shahzad's record (app_id={SHAHZAD_APP_ID})...")
    cur.execute("""
        UPDATE applications SET
            values_scorecard        = NULL,
            values_interview_result = NULL,
            values_interview_date   = NULL,
            values_interviewer_name = NULL,
            values_interview_notes  = NULL,
            status                  = 'shortlisted'
        WHERE id = %s
    """, (SHAHZAD_APP_ID,))
    print(f"  Shahzad: {cur.rowcount} row(s) cleared. Status reset to shortlisted.")

    # Step 2: Write Arsalan's scorecard to correct app_id
    print(f"Writing Arsalan's scorecard to correct app_id={ARSALAN_APP_ID}...")
    cur.execute("""
        UPDATE applications SET
            values_scorecard        = %s,
    ...
    print(f"  Arsalan: {cur.rowcount} row(s) updated. Status = rejected.")
    ...
    log_db_query(
        table="applications",
        filters=f"id IN ({SHAHZAD_APP_ID}, {ARSALAN_APP_ID})",
        rows_returned=2,
        context="fix_arsalan_shahzad_scorecard_swap"
    )

    print("Done. Both records corrected.")
