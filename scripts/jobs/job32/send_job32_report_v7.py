...
# ══════════════════════════════════════════════════════════════════
# EMAIL HTML — v7 — 7-SECTION FORMAT
# ══════════════════════════════════════════════════════════════════
...
<p style="color:#6B21A8;font-weight:bold;margin-top:4px;">v7 &bull; Updated 12-Step Framework &bull; Organisation Signal Analysis &bull; CV Quality &bull; USP &bull; 2026-03-03</p>
...
# Chart B: Score Bar Chart
html += """
<!-- Chart B: Score Bar -->
<h3 style="color:#6B21A8;margin-top:24px;font-size:15px;">B. Overall Score Comparison &mdash; Shortlisted Candidates</h3>
<table cellpadding="0" cellspacing="0" border="0" width="100%">
<tr><td bgcolor="#ffffff" style="background:#fff;border:1px solid #DDD6FE;border-radius:8px;padding:16px;">
<table width="100%" cellpadding="0" cellspacing="5" border="0">"""

bar_candidates = [
    ("Danish Hussain", 97.5, "#16A34A", "Tier A", "Out +280K"),
    ("Zain Ul Abideen &#128269;", 95.0, "#22C55E", "Tier A", "Out +80K"),
    ("Mizhgan Kirmani", 78.8, "#1D4ED8", "Tier B", "In Budget"),
    ("Arsalan Ashraf", 72.2, "#7C3AED", "Tier B", "Out +180K"),
    ("Sadia Sohail", 57.3, "#D97706", "Tier C", "In Budget"),
]
for nm, sc, c, tier, bud in bar_candidates:
    w = int(sc)
    html += f"""
  <tr>
    <td width="210" align="right" style="font-size:12px;color:#374151;padding-right:8px;white-space:nowrap;">{nm} <b style="color:{c};">&#9650;{tier[5:]}</b></td>
    <td><table width="100%" cellpadding="0" cellspacing="0" border="0"><tr>
      <td width="{w}%" bgcolor="{c}" height="26" style="background-color:{c};color:#fff;font-size:11px;font-weight:bold;padding:0 8px;white-space:nowrap;">{sc} &nbsp; {bud}</td>
      <td bgcolor="#F3F4F6" height="26" style="background-color:#F3F4F6;width:{100-w}%;"></td>
    </tr></table></td>
  </tr>"""
...
# Chart C: Radar/Star Grid — Top 3
html += """
<!-- Chart C: Top 3 Skill Depth Star Grid -->
<h3 style="color:#6B21A8;margin-top:24px;font-size:15px;">C. Skill Depth Comparison &mdash; Top 3 Candidates</h3>
<table cellpadding="0" cellspacing="0" border="0" width="100%">
<tr><td bgcolor="#ffffff" style="background:#fff;border:1px solid #DDD6FE;border-radius:8px;padding:12px;">
<table width="100%" border="1" cellpadding="8" cellspacing="0" style="border-collapse:collapse;border-color:#E5E7EB;font-size:12px;">
  <tr>
    <th bgcolor="#4C1D95" style="background-color:#4C1D95;color:#fff;text-align:left;">Dimension</th>
    <th bgcolor="#16A34A" style="background-color:#16A34A;color:#fff;text-align:center;">Danish Hussain<br>97.5</th>
    <th bgcolor="#22C55E" style="background-color:#22C55E;color:#fff;text-align:center;">Zain Ul Abideen<br>95.0</th>
    <th bgcolor="#1D4ED8" style="background-color:#1D4ED8;color:#fff;text-align:center;">Mizhgan Kirmani<br>78.8</th>
  </tr>"""

dim_labels = [
    ("D1 Functional Match (25%)", 0),
    ("D2 Demonstrated Outcomes (20%)", 1),
    ("D3 Environment Fit (15%)", 2),
    ("D4 Ownership & Execution (15%)", 3),
    ("D5 Stakeholder & Comms (10%)", 4),
    ("D6 Hard Skills / Technical (10%)", 5),
    ("D7 Growth & Leadership (5%)", 6),
]

top3 = CANDIDATES[:3]
for label, di in dim_labels:
    even = di % 2 == 0
    bg = "#F5F3FF" if even else "#ffffff"
    html += f'<tr bgcolor="{bg}" style="background-color:{bg};"><td style="font-size:12px;font-weight:bold;color:#374151;">{label}</td>'
    for c in top3:
        s = c["dims"][di]
        sc_clr = clr(s)
        html += f'<td bgcolor="{sc_clr}" style="background-color:{sc_clr};color:#fff;text-align:center;font-size:14px;font-weight:bold;">{stars(s)}<br><span style="font-size:10px;">{s}/4</span></td>'
    html += "</tr>"

html += """
</table>
<p style="font-size:10px;color:#6B7280;margin:6px 0 0 0;">&#9733;&#9733;&#9733;&#9733; = Exceptional &bull; &#9733;&#9733;&#9733;&#9734; = Strong &bull; &#9733;&#9733;&#9734;&#9734; = Partial &bull; &#9733;&#9734;&#9734;&#9734; = Weak &bull; &#9734;&#9734;&#9734;&#9734; = Missing</p>
</td></tr>
</table>
"""
...
<p style="font-size:11px;color:#9CA3AF;margin-top:28px;text-align:center;">
  Taleemabad Talent Acquisition Agent &bull; Screening Report v7 &bull; Updated Framework (12-step + Org Signal + CV Quality + USP) &bull; 2026-03-03 &bull; Confidential
</p>
</body></html>"""
...
msg["Subject"] = "Screening Report v7 — Fundraising & Partnerships Manager — Updated Framework (Job 32)"
msg["From"]    = SENDER
msg["To"]      = RECIPIENT
...
with smtplib.SMTP_SSL("smtp.gmail.com", 465) as server:
    ...
    allow_candidate_addresses(RECIPIENT if isinstance(RECIPIENT, list) else [RECIPIENT])
        safe_sendmail(server, SENDER, RECIPIENT, msg.as_string(), context='send_job32_report_v7')

print("Email sent successfully — v7 report delivered to", RECIPIENT)
