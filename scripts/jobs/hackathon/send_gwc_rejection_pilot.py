...
# Generate and prepare emails for pilot
print("\n=== GWC REJECTION EMAILS - PILOT GENERATION ===\n")
...
for name, data in candidates.items():
    ...
    emails_to_send.append({
        ...
        'subject': f"We're reflecting on your Hackathon 2026 application",
        'body': body
    })

    print(f"[OK] Generated email for {name}")

print(f"\n{len(emails_to_send)} emails ready for pilot")
print(f"PILOT RECIPIENTS: {', '.join(PILOT_RECIPIENTS)}")
print(f"\n" + "="*60)
print("Ready to send pilot. Call user for approval before proceeding.")
print("="*60)

# Save to JSON for review
import json
with open('c:/Agent Coco/scripts/jobs/hackathon/gwc_emails_for_pilot.json', 'w') as f:
    json.dump(emails_to_send, f, indent=2, default=str)
print("\nEmails saved to: gwc_emails_for_pilot.json")
