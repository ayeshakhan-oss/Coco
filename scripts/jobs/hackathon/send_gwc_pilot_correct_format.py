#!/usr/bin/env python3
"""
Send GWC decision emails with correct Taleemabad format (v8 design).
PILOT: All 6 candidates to Ayesha and Aymen for review.
Header: "PEOPLE & CULTURE • Decision of Hackathon - Taleemabad"
No logo attachment in this version - pure HTML.
"""
...
def text_to_html(text):
    ...
    for para in paragraphs:
        ...
        if para.startswith("## "):
            ...
            # Bold heading like **What we saw**
            heading = para.replace("**", "").strip()
            html += H(heading)
        elif para:
            # Regular paragraph - convert *emphasis* to <strong>
...
print("\n=== SENDING GWC DECISION EMAILS - CORRECT FORMAT (PILOT) ===\n")
...
try:
    ...
    for name, subject, html_body in emails:
        ...
        safe_sendmail(
            ...
            msg.as_string(),
            context=f"hackathon_decision_pilot_{name.replace(' ', '_')}"
        )
        print(f"[OK] Sent decision email for {name}")

    s.quit()
    print(f"\n" + "="*60)
    print(f"Sent 6 decision emails (correct format) to:")
