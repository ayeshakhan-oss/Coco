"""
Job 17 — CPD Coach
Values Interview (Zero In) Invitation — Unzeela Khalid
App ID: 2165 | Email: unzilak21@gmail.com

WORKFLOW:
  1. PILOT_MODE = True  → sends to Ayesha only for review
  2. On approval, set PILOT_MODE = False → sends to Unzeela with full CC
"""
...
# ── CONFIG ────────────────────────────────────────────────────────────────────
PILOT_MODE  = False  # True = Ayesha only; False = live to Unzeela
...
CANDIDATE   = {"name": "Unzeela Khalid", "email": "unzilak21@gmail.com", "app_id": 2165}

PILOT_TO    = "ayesha.khan@taleemabad.com"
CC_LIVE     = ["hiring@taleemabad.com", "ayesha.khan@taleemabad.com", HIRING_MGR]
...
def send_invite(to_email, to_name, cc_list=None):
    ...
    with smtplib.SMTP_SSL("smtp.gmail.com", 465) as smtp:
        ...
        safe_sendmail(smtp, SENDER, recipients, msg.as_string(),
                      context=f"job17_unzeela_values_invite_{'pilot' if PILOT_MODE else 'live'}")
...
def main():
    print("=" * 60)
    print(f"Job 17 — CPD Coach | Zero In Invite — Unzeela Khalid")
    print(f"Mode: {'PILOT (Ayesha only)' if PILOT_MODE else 'LIVE (Unzeela + CC)'}")
    print("=" * 60)

    if PILOT_MODE:
        send_invite(PILOT_TO, CANDIDATE["name"], cc_list=None)
        print("\nPilot sent to Ayesha. Review, then set PILOT_MODE = False to go live.")
    else:
        send_invite(CANDIDATE["email"], CANDIDATE["name"], cc_list=CC_LIVE)
        print("\nLive invite sent to Unzeela Khalid.")
