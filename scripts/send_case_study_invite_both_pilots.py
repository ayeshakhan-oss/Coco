#!/usr/bin/env python3
"""
Send case study invite pilot emails for both candidates to Ayesha for approval
"""
...
def send_pilot(candidate_name):
    """Send pilot email for one candidate to Ayesha"""
    try:
        ...
        msg["Subject"] = f"[PILOT] Case Study Invite - {candidate_name}"
        msg["From"] = SENDER
        msg["To"] = PILOT_RECIPIENT
        ...
        safe_sendmail(
            ...
            [PILOT_RECIPIENT],
            msg.as_string(),
            context=f"case_study_invite_pilot_{candidate_name.replace(' ', '_')}"
        )

        server.quit()
        print(f"PILOT SENT: {candidate_name}")
        return True
...
if __name__ == "__main__":
    print("Sending pilot emails to Ayesha for approval...\n")

    for candidate in CANDIDATES:
        send_pilot(candidate["name"])

    print(f"\nAll pilots sent to {PILOT_RECIPIENT}")
