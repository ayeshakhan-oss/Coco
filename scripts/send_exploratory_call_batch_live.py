#!/usr/bin/env python3
"""
Exploratory Call Batch Invites — LIVE SEND
Sends to candidates with CC to Ayesha, Hiring, Sabeena
"""
...
# CC recipients for live send
CC = ["ayesha.khan@taleemabad.com", "hiring@taleemabad.com", "sabeena.abbasi@taleemabad.com"]
...
def send_live():
    """Send exploratory invites to candidates LIVE"""
    load_dotenv()

    # Whitelist candidate addresses for bouncer
    candidate_addresses = [c["email"] for c in CANDIDATES]
    allow_candidate_addresses(candidate_addresses)
    ...
    try:
        ...
        for candidate in CANDIDATES:
            try:
                ...
                # Send to candidate + CC recipients
                all_recipients = [candidate_email] + CC
                ...
                safe_sendmail(
                    ...
                    context=f"exploratory_call_live_{candidate_name}"
                )

                sent_count += 1
                print(f"[SENT] {candidate_name} -> {candidate_email}")

            except Exception as e:
                failed.append(candidate_name)
                print(f"[FAILED] {candidate_name}: {str(e)}")
