---
name: Warm Bench Email Locked Rules — Final
description: Critical corrections from Fatima Saeed reference email (May 15, 2026). Never mention interviewer names. Never use internal jargon (GWC, values, scorecard). Use exact heading format. Start with "This is not a yes for now."
type: reference
---

# WARM BENCH EMAIL — LOCKED RULES (2026-05-30)

**Reference:** Fatima Saeed warm bench email (May 15, 2026) - **FOLLOW THIS TONE & STRUCTURE EXACTLY**

**Status:** 🔒 LOCKED IN — No deviations. These rules supercede all prior versions.

---

## CRITICAL RULES (Non-Negotiable)

### 1. OPENING LINE (MANDATORY)
```
This is not a yes for now.

But we need to tell you something about what we saw in your interview...
```

**Never deviate.** This is the first line of every warm bench email.

---

### 2. NEVER MENTION INTERVIEWER NAMES
- ❌ "During your values conversation with Jawwad Ali on April 6..."
- ✅ "You described a moment in your interview where..."
- ✅ "When we asked about X, you said..."

**Why:** Keeps focus on candidate, not on who interviewed them. More universal.

---

### 3. NEVER USE INTERNAL JARGON
**FORBIDDEN:**
- ❌ "values interview"
- ❌ "GWC interview"
- ❌ "values scorecard"
- ❌ "case study"
- ❌ "KCD"
- ❌ "warm bench"

**ALLOWED:**
- ✅ "your interview"
- ✅ "when we asked you about..."
- ✅ "the moment you described..."
- ✅ "during our conversation"

**Why:** Jargon feels internal. Candidates don't know these terms. Use human language.

---

### 4. HEADING STRUCTURE (EXACT)
```
What Stayed With Us
Where We Found Questions
Where We Want to Leave This
```

**Never use:**
- ❌ "What Stayed With Us"
- ❌ "Where We Found Questions"
- ❌ "Where We Want to Leave This"

**Why:** Consistency + brevity. Use these three headings exactly.

---

### 5. DON'T QUOTE FEEDBACK DIRECTLY
**WRONG:**
```
"The hiring manager feedback states: 'Communication could be more energetic and proactive.'"
```

**CORRECT:**
```
"What showed up was someone who sees problems clearly, cares about people inside the resistance, 
and doesn't let discomfort be an excuse to stop."
```

**Pattern:** 
- Take the feedback intent
- Reframe it as observation of WHO THEY ARE
- Use positive language that affirms character
- Don't repeat negative phrasing

---

### 6. WORD COUNT
- **800-1100 words MANDATORY** (verified count)
- If <800: expand with more interview moments
- If >1100: trim redundancy, keep substance

---

### 7. TONE CHECKLIST
- [ ] "We" voice (never "I")
- [ ] Warm + observational (not clinical)
- [ ] Specific interview moments (never generic)
- [ ] No prescriptive advice ("You should...")
- [ ] No life-coach language
- [ ] Vulnerable (show company's perspective, not just candidate's gap)
- [ ] Poetic (closing P.S. ties back to powerful moment)

---

### 8. STRUCTURE (3 SECTIONS + P.S.)

**Section 1: What Stayed With Us**
- 2-3 specific interview moments
- Show what impressed the panel
- Use "we kept discussing this afterward" / "the panel talked about..."
- Affirm character, not just competence
- Example: "You didn't just solve the problem. You cared about the people impacted."

**Section 2: Where We Found Questions**
- Acknowledge the interview was strong
- Explain the decision was narrow/situational
- Don't apologize; be matter-of-fact
- Frame as "timing didn't align" not "you weren't good enough"
- Show company vulnerability too

**Section 3: Where We Want to Leave This**
- Warm bench positioning (genuine, not obligatory)
- Express interest in future connection
- Affirm the kind of person they are
- End with warmth, not finality

**P.S.: The Powerful Echo**
- Reference ONE powerful moment from interview
- Tie it back to who they are
- Emotional, brief, memorable
- Candidate should screenshot this part

---

### 9. SUBJECT LINE RULES
- **Poetic, story-based, tied to specific interview moment**
- ✅ "When You Stop a Meeting to Protect Your Team"
- ✅ "When Personal Experience Becomes Professional Calling"
- ❌ "Huma Mumtaz - Fundraising & Partnerships Update"
- ❌ "Interview Feedback"

**Pattern:** [When/The moment] + [specific action from interview]

---

### 10. SIGNATURE
```
Warm regards,
People and Culture Team
Taleemabad

hiring@taleemabad.com | www.taleemabad.com

Sent on behalf of Talent Acquisition Team by Coco
```

**Never:**
- ❌ Use individual name
- ❌ Use different closing
- ❌ Forget "Sent on behalf of Talent Acquisition Team by Coco"

---

## EXAMPLE PARAGRAPH COMPARISON

**WRONG (Too clinical):**
```
Your GWC scorecard feedback indicated you could improve communication energy and proactivity. 
While you demonstrated technical knowledge, another candidate was more aligned with our 
current needs in terms of presentation style.
```

**CORRECT (Human + observational):**
```
Your interview created genuine discussion. There was real appreciation for how you show up—
for the integrity you bring, for the way you care about the people you work with, for your 
willingness to learn things that scare you. None of that is ambiguous. The panel saw it.
```

---

## REFERENCE: FATIMA SAEED EMAIL (MAY 15, 2026)

Read this email in full whenever drafting a new warm bench email. It is the gold standard:
- Opening: "This isn't a yes for now."
- What Stayed With Us: 4 specific moments (coaching scenario, closing questions, notice period honesty)
- Where We Found Questions: Warm + clear about decision being situational
- Where We Want to Leave This: "We'd genuinely like to stay connected"
- P.S.: Ties back to "That coach you described from your TFP fellowship..."

**Copy this structure exactly for every warm bench email.**

---

## 10. NEVER MENTION "ANOTHER CANDIDATE" (CRITICAL)
- ❌ "Another candidate's background was a tighter fit"
- ❌ "Someone else's experience profile matched better"
- ❌ "We had another candidate who..."
- ❌ Comparative language of any kind

**WHY:** Demotivates candidates. Makes them feel they "lost" to someone.

**ALLOWED:**
- ✅ "We're moving into a specific phase where we need X capability"
- ✅ "Your profile is strong, but not the exact match for what this chapter requires"
- ✅ "It's about fit between your strengths and our immediate strategic need"
- ✅ "The decision reflects our specific moment, not your capability"

**Pattern:** Focus on YOUR NEEDS, not their superiority. Frame as "what we need right now" not "someone was better."

---

## LOCKED IN AS OF
- **Date:** 2026-05-30
- **Source:** Fatima Saeed email (May 15, 2026) + user feedback + quality review protocol
- **Status:** 🔒 NO FURTHER CHANGES — This is the final format

---

**Apply these rules to ALL warm bench emails going forward. No exceptions.**
