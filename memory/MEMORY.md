...
## 🆕 NEW — BENEFITS PULSE CHECK GOOGLE FORM + 🔑 FORMS API IS NOW ENABLED (2026-09-03)
- **[Benefits Pulse Check Google Form (2026-09-03)](project_benefits_survey_form_2026_09_03.md)** — anonymous **12-question staff survey** behind Ayesha's **Draft T-Flex 2027** flexible-benefits paper. Form `1FJeLmuKHw-XVilArMFkodZobTMGep_fLcBER-_yFWmk`, owned by ayesha.khan@, [live link](https://docs.google.com/forms/d/e/1FAIpQLSd-eStYElezDh5PuxaqtE0KstPrkaNPEkKP_T8pLuVGH22y2Q/viewform). Builder `scripts/pnc/build_benefits_survey_form.py` (source of truth), patcher `scripts/pnc/patch_benefits_survey_form.py info <indices>` — **patch the existing ID in place, never re-run the builder** or you get a second form on a new URL. 🔑 **THE GOOGLE FORMS API NOW WORKS HERE:** the probe returned **`SERVICE_DISABLED`, not a scope error** — the `drive` scope on `token_sheets_broad.json` is **already accepted by the Forms API**, and Ayesha enabled the API on GCP project 954828175525 on 2026-09-03. **Read the 403 body before declaring a capability dead:** `SERVICE_DISABLED` = one console click; `insufficient scope` = a real re-consent. (Slides API on the same project is still OFF.) ⚠️ **API cannot do:** checkbox "select at most N" validation (description text only, must be set in the UI), email-collection/response settings, and clearing an item description needs an explicit `description: ""` not an omitted key. 🔴 **Ayesha's format call:** the hypothetical worked example (PKR 200,000/yr pool drawn via a wallet) belongs in the **form description at the top**, NOT on the question — she rejected it on Q8. **Reusable rule for exploratory staff surveys:** the no-promise framing ("research exercise, not an announcement; no change proposed, approved or budgeted; answers do not create an entitlement") goes in the form description where it cannot be skipped, and every hypothetical figure is hedged in the sentence that introduces it. The name **T-Flex is deliberately absent** from the form. ⚠️ **Open, undecided:** "Cosmetics" in Q2 reads inconsistently; and there is **no permanent/contractual field**, so Q3 cannot separate *doesn't want it* from *not eligible* for the 69 contractual staff.

## 🆕 NEW — FUNDRAISING CAPABILITY DOC SET + THE "NO NAMES, NO INVENTED FACTS" DOC RULES (2026-09-06)
- **[Fundraising Capability Doc Set (2026-09-06)](project_fundraising_capability_docs_2026_09_06.md)** — three artefacts for Ayesha's **P&C Buddy Fundraising theme**, all owned by ayesha.khan@ in folder `1R0ZZCZ81v-cgHr22PbRp_KFJiCJ91_Kj`: **working guide** `1ERHy6vp_T0ERorKKEvvdrJUPtwx0BCYJHsyIqcp7IJ8` (master, ~5,300 words), **facilitator guide** `1IkXbBZeLPq9KXBOaL6kmJcU3ULhHWTYmEJmDNhkwV8I` (7 pages), **deck outline** `1af_qcCZxIRN4YOS6J-DrRSV1Y6EVl4PCCO-UcTGA5gk`, plus the built **Google Slides deck** `10jpoN7IsA0Az-zNeRx1kkJq2blyceIQKP69RlcGGSeo` and its **.pptx** `1nFJJvs4Fdv6dE0xvXdg5jsaEK8Q1JAjm`. Sources in `docs/pnc_buddy/`, deck builder `scripts/pnc/build_fundraising_workshop_deck.py` — **re-render to the same IDs, never create a second copy**. 🔴 **NO PERSONAL NAMES, EVER** — Ayesha said it twice, the second time in caps after I reintroduced them because her own brief referenced people by name in required content. **A brief that mentions someone by name is not permission to name them in the document.** Role labels only: *Senior Manager Fundraising*, *Track A*, *Track B*; placeholders read `[To be added/validated with Fundraising / Finance / Programmes]`. Scan the **rendered** Doc and the deck's speaker notes, not just the markdown. 🔴 **NO INVENTED TALEEMABAD FACTS** — never fabricate a figure, result, donor example or impact claim; the master carries **15 placeholders on purpose** and the budget exercise table is deliberately blank. 🔴 **INTERNAL WORKING-DOC TONE, not consulting register** — "we / our team / for our context", hedge every proposal ("we could", "one way we can approach this"), leave real questions open, and BAN "the key is", "at its core", "X is not Y, it is Z", "the most common mistake", motivational one-liners and the same three headings repeated under every item. Keep external research, our proposal and what still needs validating clearly separate. ⚠️ **Slide 9 lesson:** a list of capability names is **not** the capabilities section — each box needs the name **plus** what we would expect to see in practice. ⚠️ **Vitol takes no unsolicited applications** (fetched, not inferred) so it is a cultivation problem, not a proposal problem. ⚠️ **Slides API is NOT enabled on this GCP project** (403) — build .pptx with python-pptx and let Drive convert. 🔑 **HOW TO ACTUALLY SEE A DECK/PDF HERE:** poppler is not installed, so export the Drive file as PDF → render pages to PNG with **PyMuPDF** (`fitz.open(pdf)[i].get_pixmap(dpi=110).save(png)`) → Read the PNGs. That is a **genuine visual check** and it lifts the Rule 14 limitation for anything Drive can export.

## 🆕 NEW — P&C BUDDY MEETING TRACKER SHEET + THE "TASK LIST ONLY" FORMAT RULE (2026-09-06)
- **[P&C Buddy Meeting Tracker Sheet (2026-09-06)](pnc_buddy_meeting_tracker_sheet_2026_09_06.md)** — Ayesha's meeting-notes tracker: spreadsheet `17eb8v55YQOKIiqpd3c2Bq6dDM8_6Wu9EzgsKh0WHr6w` **"P&C Buddy Tracker Sheet"**, owned by ayesha.khan@, **one tab per P&C Buddy counterpart** (first tab `Sabeena`, built from the 4 Sep Fathom 1:1). Separate from the four-theme P&C **Doc** `1CWPHvRdTtAwJ3gdHyVdwP1MWzA2C4NwM1ZimtNCiylI`. Columns `Date | Topic | Task | Owner | Priority | Done (checkbox)`; ticking Done strikes through and greys A:E via `=$F2=TRUE`; a second rule reddens `CRITICAL`. 🔴 **THE FORMAT RULE, learned over FOUR rejected passes on this one sheet:** Ayesha rejected (1) one row per task with topic+minutes repeated down the page, (2) paragraph minutes written once per topic block, (3) even a one-line summary column — and accepted only **tasks, nothing else**. Topic shows once per block for grouping; **the full minutes survive ONLY as a hover NOTE on the topic cell** (`updateCells` + `fields:'note'`), which keeps the record at zero visual cost. **Apply to any tracker or notes artefact for her: no narrative prose in a cell, default to less.** Builder `scripts/pnc/build_pnc_buddy_tracker_sheet.py` — bare run **aborts if a sheet of that name exists** (never create a second), `--update` rewrites the tab in place and must wipe **values AND notes** (`values().clear()` alone leaves stale hover notes). ⚠️ **`gspread` is NOT installed in `.venv`** — use the raw `googleapiclient` sheets/drive v4 clients. ⚠️ **Column indices shift when a column is dropped** — re-check every conditional-format range, the validation range and the `=$F2` formula after any layout change. Transcript is a lossy Fathom auto-transcript: 4 open ⚠ items flagged inline rather than guessed (salary "400 to 450" **units unstated** · coaching support **Zaheb or Zohaib** · **Momina vs Momna Tariq** may be two people · Nawal passage switches he/she and Nawal/Namal). Structural API checks confirm values/notes/rules, **not appearance** (Rule 14).

## 🆕 NEW — RM INTERNAL OPENING: 25 CASE STUDIES ANONYMISED + TRACKER (2026-08-31)
- **[🔴 RM round: marked scripts, feedback emails, documentation folder (2026-09-08)](rm_marked_scripts_sheet_2026_09_08.md)** — the whole below-70 workflow end to end. **Marked scripts** sheet `1tSl69wejsDdOBHppfqQXOpCC1HEdhCXNTBltcaNBt8k` (exam-style, Q1-Q10 with marks, what's good, what's missing, cap wording). **Individual workbooks** per candidate, privacy-asserted at build time. **8 rejection letters sent live** with the benchmark + their own script attached. **Documentation folder** `1hIhLcu9TSLVptef9STFKZQ6CQ6lMebVn` + index `1_-H0hLYj0pujY-9Jfw8zEhXtPTdhGFBuJGMtfQhZ-bo` (25 named originals, 53 files, every round document linked). 🔴 **Three link failures in one session, one root cause:** the evaluation report Ayesha wanted **only ever existed as an email body** (2 Sep, `19_7fqUBGwrzBbJJZkrdR49zXB_HsLgt6IIyYOofqw88` now), so no Drive search could find it and I re-pointed the index twice instead of asking which artefact she meant. Two other artefacts still carried the **superseded first pass** (mean 88.3, nobody below 70): the evaluation Doc, **rewritten in place** so shared links resolve to the truth, and a DRAFT-for-QA benchmark PDF. **The tracker's Scores tab is the last one still wrong.**
- **[🔴 RM below-70 MARKED SCRIPTS sheet (2026-09-08)](rm_marked_scripts_sheet_2026_09_08.md)** — exam-style marking for the **8 below the published 70% bar**, sheet `1tSl69wejsDdOBHppfqQXOpCC1HEdhCXNTBltcaNBt8k`, builder `scripts/reports/build_rm_marked_scripts_sheet.py`. One tab per candidate: total vs the bar, examiner's comment, explicit **What's good** / **What's missing** blocks, the **cap wording that fired**, then Q1-Q10 with max, awarded, 0-5 anchor, what they wrote and what would have scored higher. Marks = anchor converted pro-rata to the case's printed points; verified twice in code (marks sum = total AND anchors x points = total). 🔴 **The tracker's own Scores tab still holds the SUPERSEDED first pass** (mean 88.3, nobody below 70, Javeria 91.5 vs her real 64) — **a re-mark must be written back to every artefact, not just the report.**
- **[RM internal case study tracker (2026-08-31)](project_rm_internal_case_study_tracker_2026_08_31.md)** — **25 internal submitters** for the Regional Manager opening. The 27 Aug 1:00 PM deadline was **extended by Ayesha on 25 Aug to Fri 28 Aug 11:59 PM** with submissions redirected to **hr@taleemabad.com**; all 25 landed inside it. Count cross-checked two ways (IMAP sweep of ayesha.khan@ + the 25 HR acknowledgements). Case studies anonymised as **RM-01..RM-25** (md5-of-name order, so code order != submission order), uploaded to Drive, tracked in sheet `1xtQxfblMXmA5IpnvnABkK5bvmhPMK5_Q6Z1wDiI59A8` with a **Master Key** tab (code + real name + resume) and a names-free **Evaluation (share this)** tab. 🔴 **Reusable anonymisation recipe:** PDF via **PyMuPDF redaction** (never flatten to text — it destroys the tables the rubric scores); DOCX/PPTX names are routinely **split across runs** (`S|hafaq T|ahir`), so join a paragraph's runs before replacing or a plain string replace silently misses them; deny-list `islam din khan ali syed muhammad abdullah anwar` because they collide with real case content (Islamabad, stakeholder names). 3 docx needed `[Content_Types].xml` repaired (missing `Default Extension`, one literally `.undefined`) before python-docx would open them. **Verified:** 0 identifiers left across all 27 outputs, all PDFs text-based (check not vacuous), word-diffs show only names removed. ⚠️ **Limit (Rule 14): text extraction cannot see inside embedded images** — several PDFs carry 10-47 charts/screenshots, not OCR-verified. 🔴 **Flags:** Shafaq Tahir sent **NO resume** (both files are the case study); Bushra Karim submitted via Drive links; Khadija Akbar's **Fri 28 Aug** version is the one to use; Ashas Khan + Misbah Iqbal sent resumes only on 31 Aug. **Nothing scored yet** — Rule 17 order stands: benchmark answer key first, Ayesha QAs it, then read submissions.

## 🆕 NEW — SMG DEBRIEF INVITES BATCH 2, LIVE TO 6 (2026-08-31)
- **[SMG case study round 2/3/4 + batch-2 debrief invites](project_smg_case_study_round2_2026_08_24.md)** — Consolidated 9-candidate report piloted 31 Aug 02:33 (+ single PDF 02:39). Final order: **Khushal Kakar 98 · Furqan 93 · Hania 90 · Vaneeza 82 · Shafaq 75 · Lamis 73 · Kanooz 62 · Ali Wajdan 49 · Rimsha 45.** The **six above 70 were invited to the case-study debrief LIVE 2026-08-31** (`scripts/send_case_study_debrief_smg_batch2_pilot.py`, body cloned verbatim from the 18 Aug batch-1 send; booking link `calendar.app.google/VXJ1qxddrMVakeFAA` — **a different link from batch 1, never reuse one across batches**; Cc waqas.tanveer@ + ayesha.khan@ + hiring@ + ali.sipra@). Verified after send: exactly 1 invite per recipient, 0 bounces. 🔴 **Markaz names are unreliable for BOTH halves, and so are CV filenames:** Khushal's Markaz record AND his CV filename say "Khushal **Khan**" while he signs himself "Khushal **Kakar**" in all 3 of his own emails (went live as Kakar, flagged in pilot); Vaneeza's Gmail display name says "Veniza" while Markaz AND her CV filename say "Vaneeza" (used Vaneeza). **A display name and a CV filename are both self-authored and can disagree — say which you used and why, and flag it before a live send.** 🔴 **Reusable check: per-recipient IMAP scan of Sent Mail for the invite's own subject BEFORE drafting and AFTER sending** — catches duplicate sends and silent omissions. Still unassessable: **Muhammad Ahmad Taj (3971)**, files never arrived.
...
## 🆕 NEW — OPL PROJECT CONTRACT: AYESHA NOREEN, AUDIO MONITORING (LIVE 2026-08-24)
- **[Ayesha Noreen — OPL Project-Based Audio Monitoring contract (2026-08-24)](contract_ayesha_noreen_audio_monitoring_2026_08_24.md)** — First **fixed-fee** contract built off the NIETE project master. Ms. Ayesha Noreen · Audio Monitoring Officer · 24 Aug → 3 Sep 2026 (11 days) · **PKR 36,667** (50k/15d pro-rated; **derivation kept out of both documents per Ayesha**) · reports to Abubakr · signed by HOD **Sabeena Abbasi**, not the COO · lunch + business-travel clauses stripped. 🔴 **THREE MASTER TRAPS:** (1) **`Per month` is hardcoded beside the compensation fill spot and is NOT a yellow field** — it survives a normal fill and is factually wrong on any fixed-fee contract; replaced with "for the complete assignment inclusive of Tax" and now asserted in the builder. **Check this on every fixed-fee build.** (2) the project line is hardcoded to *National Institute of Excellence in Teacher Education* — overwritten to "Assessments". (3) acceptance labels (`Signature:` / `Contract Acceptance Date:` / `Direct Report to:`) are not bold in the master; `Direct Report to: <name>` is a **single run** so it must be split, and bold set on the RUN or Drive's PDF flattens it. 🔴 **SEND-HOOK GAP FIXED:** `pre_contract_send_hook.py` could only render email HTML from multi-candidate scripts (`render(key)` + `COACHES`); a **single-candidate script with a zero-arg `render()`** fell back to static source scanning and threw **FALSE "compensation is not bold" HARD BLOCKS** on a correct email. Added `_takes_arg()` + a zero-arg branch. **Design 3 gained four optional prose slots** (`WELCOME_EXTRA` · `ATTACHMENT_NOTE` · `CLOSING_EXTRA` · `CLOSING_WELCOME`) so programmes can vary wording without touching the locked layout; NIETE passes its originals and renders identically. **Template-wide (affects NIETE too):** "HR team" → "People & Culture team", "HR portal" → "People & Culture portal". 🔴 **Rule 1 proved again: every round here was a visual defect Ayesha caught by eye AFTER structural checks reported clean.**

## 🆕 NEW — INTERNAL ANNOUNCEMENT EMAIL (Skill 01 type #7, LIVE 2026-08-20)
- **[Internal Announcement — Skill 01 type #7 (2026-08-20)](internal_announcement_email_type_2026_08_20.md)** — New email type at Ayesha's request. 🔴 **AUDIENCE IS TALEEMABAD STAFF, NOT CANDIDATES** — internal job openings, new joiners, new programmes, org changes. Lives in Skill 01 as she asked, but declared an **internal type** with its own exemption block, because the candidate harness rules would otherwise produce an 800-word all-staff email opening "This is not a yes for now." that refuses to name the person it is announcing. **DISAPPLIED:** 800-word minimum · mandatory opening line · future-promise ban · no-names ban · candidate-jargon ban · feedback widget. **STILL ENFORCED:** no em dashes · v8 imported from `v8_template.py` · collective voice · `safe_sendmail()` · pilot to Ayesha first · clean subject live · no fabricated facts. Target **150-400 words**; content varies per send (Ayesha writes/approves it), only the LAYOUT is locked. Added to `v8_template.py`: `EYEBROW["announcement"]` (reads "INTERNAL ANNOUNCEMENT" as a visible tripwire if it ever reaches an external inbox) + shared helpers **`UL(items)`** and **`PL(text)`**. Script `scripts/send_internal_announcement_pilot.py` — **⚠️ keep `announcement` in the filename, never `warm_bench`/`gwc`/`values`/`rejection`** (the hook infers type from filename and would HARD BLOCK a 243-word notice for failing the 800-word candidate rule). Hook gained `announcement` inference — **non-obvious: when the hook cannot infer a type it returns early and silently skips the `[PILOT]`-prefix-in-live guard too**, so registering the type is what turns that guard back on. **First use LIVE 2026-08-20:** Regional Manager (RM) internal opening → TO `all@niete.edu.pk`, CC ali.sipra@ + hiring@ + ayesha.khan@ + bilal@niete.edu.pk + asma.zaheer@niete.edu.pk. 🔴 **Two process lessons:** (1) **when a stage is removed from a hiring flow, the deadline attached to it does not vanish with it** — the case study moved from "sent after resume screen" to "shipped with the announcement", so the separate 5-working-day window had to fold into ONE deadline (Thu 27 Aug, verified = exactly 5 working days) instead of leaving the old Fri 21 Aug date standing. (2) **Never resolve a bare first name from the repo** — Ayesha wrote "bilal" with no domain; grep returned **11 different Bilal addresses**, one of them a live Job-42 candidate. Ask. Skill file: [07_internal-announcement-email.md](../.claude/skills/01_candidate-communication/07_internal-announcement-email.md).

## 🆕 NEW — GM-LAHORE VALUES INVITE (USHNA FAWAD) + 🔴 SEND-HOOK GAP (2026-08-24)
- **[GM-Lahore values invite + verified links + hook gap](gm_lahore_values_invite_ushna_2026_08_24.md)** — Pilot 24 Aug, **LIVE 31 Aug**, **she booked her Zero In call 1 Sep**. 🔑 **Verified GM-Lahore invite links** (each proved by fetching the URL and reading its `<title>`): JD `1ui-Y_jTr9x...` = "Growth Manager Lahore" · prep guide `1TBbBAimVX9...` (role-agnostic) · booking `calendar.app.google/WMdp99dVmqPuuMvp9` = **"Zero in Call for Growth Manager Lahore"**. LIVE CC = ayesha + hiring@ + waqas.tanveer@. 🔴 **TRAP: `scripts/jobs/job39/send_growth_manager_invites_batch.py` sits in the job39 folder but is configured for Job 41 KARACHI** — copying it books a Lahore candidate into the Karachi schedule; the real Lahore reference is `scripts/send_growth_manager_values_invite_pilot.py`. Never trust folder or filename, fetch the link. 🔴 **Duplicate application (Kanooz pattern):** Ushna existed twice on job 39 — cand 2987/app 3703 LinkedIn stub **silently rejected 9 Jul** (null reason) vs cand 3349/app 4141 real CV; search by name AND phone AND LinkedIn URL. She was still `status='new'` (unscreened) when invited. 🔴 **HARNESS GAP — Layer 3 send-time validation NEVER FIRES:** `pre_send_validation_hook.py:104` early-returns unless `tool_name` contains "send", but the PreToolUse matcher is **"Bash"**, so it validates nothing on any script send. CLAUDE.md called it "✅ ACTIVE"; corrected. **Not yet fixed.**
...
- **[SMG Case Study Round 2 (2026-08-24)](project_smg_case_study_round2_2026_08_24.md)** — 6 new submitters scored on the same benchmark/rubric: **Furqan Afzal 93 · Hania Khan 90 · Shafaq Syed 75 · Lamis Maniar 73** (proceed) · Kanooz Siddiqui 62 borderline (pool not thin → no) · Rimsha Taj 45 (no) · **Ahmad Taj not assessable — his "attached ZIP" never arrived anywhere**. 🔴 **The dataset has TWO coaching columns**: `audio_coaching_sessions` sums to **1**, `coaching_started` sums to **118** across 35 adopters — reading the wrong one makes the flagship feature look dead. 🔴 **Check that a per-user ratio has users on both sides**: Rimsha's "11.1 active days" divides all-country active days by feature-session count, so it ranks features in **inverse order of use**. 🔴 **Run the sent-vs-submitted diff before closing a round** — it found **Zeshan Nawaz (3921) + Bilal Sadiq (4051)**, sent 7 Aug, never submitted, **never nudged**, still `shortlisted`. Link submissions leave **no attachment** — the URL lives only in `applications.case_study_submission` in Neon. Raw CSVs are **not in the repo**; recovered the 546-row master from a candidate workbook and revalidated it against every benchmark figure before recomputing anyone's numbers.
...
- **[🔒 Proving an interview actually happened (2026-08-29)](reference_interview_completion_evidence_2026_08_29.md)** — **values/Zero-In calls → Fathom recap** (candidate email sits in the SUBJECT, not the headers); **case-study debriefs → Read AI "Read Meeting Report" + Rumi "Meeting Summary"**. Searching Fathom for a debrief returns zero and makes a live pipeline look dead. Markaz records none of it. **No report ≠ the call did not happen** (a report exists only if a note-taker joined; Abdul Wahab's came from his OWN Read AI). Count people not events; the LAST "Updated invitation" wins. Full-mailbox header scans need UIDs + checkpointing — the connection drops.
...
- **[🔒 Case Study Outcome, below the benchmark (Skill 01 type #8, 2026-09-08)](case_study_outcome_email_type_2026_09_08.md)** — Rejection for a candidate who reached the case-study stage, **submitted**, and missed the **70% benchmark that gates final interviews**. 800+ words, full types-1-4 rule set. **Evidence from exactly two sources:** their own submission plus the benchmark answer key written before any submission was opened (declared as `SUBMISSION_SOURCE`/`BENCHMARK_SOURCE` + 3 verbatim `ANCHORS`). **The 70% rule is STATED; the individual score is NEVER printed** (no NN/100, no band, no ranking, no pool comparison — the eval reports rank the pool and that framing is internal). "case study" permitted candidate-facing, as for type #6. **No conversation references** — a booked call is not a held interview. 🔴 **The gate lives INSIDE `scripts/send_case_study_outcome_pilot.py`** and raises before SMTP, because the Layer 3 hook is inert; all 17 checks were proven to fire against broken drafts before piloting. ⚠️ **A repo-only scan under-counts a cohort** — reconcile against every sent report, not the latest one. ✅ **LIVE 2026-09-08** to all 5 Job-42 SMG candidates (CC waqas + ali.sipra + hiring + ayesha.khan; 1 copy each, no PILOT leak, verified by IMAP). 🔒 **TONE HARNESSED** in the shared eval harness (`HARSH_LANGUAGE` + `CORPORATE_BOILERPLATE`), so the CLI and the **Railway webapp** both enforce it; decision sentence locked to "did not meet the 70% benchmark **on this occasion**". 🚀 **Deployed in the webapp**: `EMAIL_TYPES`, `tone_rules.py` (+ jargon exemption), `draft_prompt.py`. 🔴 **An evaluation record is not the source** — fetch the submission and check the letter against it; doing so found 5 defects after 2 pilots (1 factual, 4 drifted quotes). `_quotes_are_verbatim()` now hard-blocks any quote absent from the candidate's own files. 🔒 **Skill 01's eight type files are numbered 01-08 by type** so the folder reads in registry order and `06_case-study-update` (pending) cannot be mistaken for `08_case-study-outcome` (the rejection). 🔴 **No sub-skill concept exists** — a skill is only `.claude/skills/<name>/SKILL.md`, so the filename and sort order are the whole UX. 🔴 **A bare colon in a YAML description silently kills the frontmatter** and the skill loses its description; `yaml.safe_load` after every edit. ⚠️ `.claude/skills/` is never deployed to Railway; ⚠️ Skill 07 still has no frontmatter.
- **[Case Study Update — Debrief-Pending (Skill 01 type #6, 2026-08-13)](case_study_update_email_type_2026_08_13.md)** — Short status note to a candidate who **already submitted their case study** and is waiting on a debrief decision. Lives under **Skill 01 (candidate communication)** as a sibling of the Warm Hold Decision-Pending Update (type #5), created at Ayesha's request 2026-08-13. Says three things only: thanks for the work, we are still mid-interviews so nothing is decided, and **"we expect to share an update with you on the case study debrief interview call by [timeline]"** (Ayesha's exact wording — say "case study debrief interview call", NEVER just "debrief"). **EXEMPTIONS (inherited from type #5):** no "This is not a yes for now." opener, the dated promise is REQUIRED (future-promise ban inverted), 800-word minimum does not apply → target **120-250 words**. **NEW exemption scoped to THIS TYPE ONLY:** "case study" is permitted candidate-facing language (it is the candidate's own deliverable, and Ayesha names it on the values call) — this does NOT loosen the jargon ban anywhere else. **BANS:** no evaluation of the submission, no direction hints, no apology theatrics, no new asks, no mention of other candidates. Subject 🔒 `A Quick Update from Our Side` (deliberately a sibling of type #5's "A Quick Note from Our Side"). Layout: v8 via `EYEBROW["case_study_update"]`. Script: `scripts/send_case_study_update_pilot.py`. **⚠️ NEVER rename that script to contain `warm_bench`/`gwc`/`values`/`rejection`** — the send-time hook infers type from the filename and would HARD BLOCK this 133-word note as an under-length 800-word feedback letter. **Default CC (Ayesha 2026-08-13):** ayesha.khan@ + waqas.tanveer@ + ali.sipra@ + hiring@. First live use 2026-08-13: 4 Job-42 SMG case-study submitters (Arshan Bilal, Junaid Ali, Arooj Khalid, Yusra Amjad). **Guardrail:** if the promised timeline will slip, send a fresh note BEFORE it passes. Skill file: [06_case-study-update-email.md](../.claude/skills/01_candidate-communication/06_case-study-update-email.md).

## 🆕 NEW — CASE STUDIES: SMG + GROWTH MANAGER, DERIVED FROM HOG (2026-07-31)
- **[🔒 RM benchmark-comparison documents + debrief invites (2026-09-07/08)](project_rm_internal_case_study_round_2026_09_02.md)** — Question-by-question comparison of all 25 submissions against **Revision 2** of the answer key, split into two PDFs at the 70 line (`scripts/reports/make_rm_benchmark_comparison.py` builds, `send_rm_benchmark_comparison.py` mails). Ayesha's five Rev-2 additions are badged **REV 2** in the documents: *Where from* column in Q1, Q2 reframed off a flat no, evidence→intervention rule in Q3, corrected Q4 arithmetic + **Very High activities never paused**, fifth column restored in Q8. 🔑 **The comparison surfaced what the totals hid: Q2 and Q4 both average 62% while the other seven sit at 78-85%.** Only **2 of 25** reached the key on Q2 and **17 of 25** misread the below-grade-level figures as population shares (34+75=109 proves they cannot be); **6 of 25** showed no Q4 arithmetic at all. 17 debrief invites live 2026-09-07 to everyone at/above 70. Developmental analysis for the 8 below: `scripts/reports/send_rm_below70_question_analysis.py`.
- **[🔒 LESSON — anchor floors inflate until a scale cannot fail anyone (2026-09-02)](lesson_scoring_anchor_floor_inflation_2026_09_02.md)** — I marked 25 case studies and put ALL of them above the 70% bar; Ayesha challenged it and was right. On four of nine questions the WORST answer of 25 still scored 70%. Cause: anchors of 5/3/1 convert to a 20% FLOOR, plus crediting structure over correctness, plus never applying the benchmark's own caps. **Rule: convert on the full 0-5 scale with a real zero, write the caps as mechanical rules BEFORE scoring, tick every required element individually, and check `min(anchor) per question` before reporting.**
- **[🔒 RM Internal Case Study Round — all 25 scored (2026-09-02)](project_rm_internal_case_study_round_2026_09_02.md)** — 25 internal submissions vs benchmark Rev 2. **All 25 clear the published 70% bar** (mean 88.3, low 70.0) so a SECOND cut is needed; 70% was announced and cannot be raised retroactively. Anonymisation lessons: docx names split across runs (join runs first), `.paragraphs` never reaches text boxes, never deny-list a surname that collides with a common word — two real leaks found that way. Score writer must be atomic+utf-8 (a cp1252 error truncated scores.json to 0 bytes). Report: `docs/case_studies/rm_evaluation_report_2026_09_02.md` + Google Doc `1suHQOhKzAjjBe26uPjSpHkl0EoSVBGqkhtReqjs8QMc`.
