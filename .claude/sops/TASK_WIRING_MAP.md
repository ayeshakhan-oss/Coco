...
| **Screen a TECHNICAL / engineering role** | .claude/RULES.md | .claude/skills/02_candidate-evaluation/technical-screening.md | REPORT_FORMAT_LOCKED.md | nugget_technical_screening_adopted_2026_09_15.md | 🔴 Do NOT screen by hand → read Nugget's result (`scripts/screening/read_nugget_screening.py`) → filter `is_current` → attribute to Nugget → never report UNUSABLE as a rejection |
...
| **Create decision brief** | .claude/RULES.md + Rule 1.11 | .claude/skills/03_operations/decision-briefs.md | N/A (custom) | feedback_decision_brief_hyperlinks.md (CV links mandatory) | Check Markaz → Draft → Verify all names linked → Self-QA |
| **Generate attendance report** | .claude/RULES.md | .claude/skills/03_operations/attendance-reports.md | attendance_report_complete_template.md | attendance_report_complete_template.md (colors, sections, no grid) | Query Teams/Markaz → Build → Self-QA |
